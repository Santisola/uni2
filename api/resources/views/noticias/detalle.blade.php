...
@section('main')
    <div id="noticia" class="container-2xl mb-5">
        <h1 class="text-center texto-violeta font-bold text-3xl mb-5">{{ $noticia->titulo }}</h1>
        ...
        <ul class="mt-5">
            ...
            <li class="rounded border border-double w-fit border-4 border-violeta px-3 px-2 bg-violeta text-white">Status: Publicado</li>
            @else
                <li class="rounded border border-2 w-fit border-violeta px-3 px-2 texto-violeta">Status: Borrador</li>
            @endif
        ...
        <h2 class="mt-5 text-2xl font-bold texto-violeta">Nota:</h2>
        <p class="mt-2 whitespace-pre-line mb-10 break-words detalle">{{ $noticia->contenido }}</p>
        <a href="{{ route('noticias.editarForm', ['noticia' => $noticia->id_noticia]) }}" class="w-full py-3 px-2 my-3 rounded border border-violeta text-center texto-violeta hover:ease-in-out hover:text-white transition duration-300 md:w-fit">Editar</a>
        <section id="comentarios">
            <h2 class="text-center texto-violeta font-bold text-3xl mb-1 mt-5">Comentarios</h2>
            @forelse($comentarios as $comentario)
                <div class="comentario-container">
                    <dl class="flex items-center">
                        <div class="img-perfil">
                            @if($comentario->verificado->imagen)
                                <img src="{{ asset(str_replace('public/','', $comentario->verificado->imagen)) }}" alt="Imagen del verificado">
                            @else
