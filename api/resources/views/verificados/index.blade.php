...
@section('main')
    ...
    @forelse($usuarios as $usuario)
        <div class="flex flex-wrap justify-between items-center w-full py-5 px-10 mb-5 tableta verificados shadow-lg rounded-lg">
            ...
            <div>
                <form action="{{ route('verificados.verificar', ['usuario' => $usuario->id_verificado]) }}" method="post" class="flex flex-col">
                    @csrf
                    @method('PUT')
                ...
                @if($usuario->deleted_at)
                    <form action="{{ route('verificados.restaurar', ['usuario' => $usuario->id_verificado]) }}">
                        @csrf
                ...
                @else
                    <form action="{{ route('verificados.eliminar', ['usuario' => $usuario->id_verificado]) }}" method="post">
                        @csrf
                        @method('DELETE')
